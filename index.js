...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '鈱� Calmae garai to fazendo aq 鈱�',
				...
				error: {
					stick: '[鉂梋 Falha, ocorreu um erro ao converter a imagem em um adesivo, tente novamente 鉂�',
					Iv: '鉂� Link inv谩lido 鉂�'
				},
				only: {
					group: '[鉂梋 Este comando s贸 pode ser usado em grupos! 鉂�',
					ownerG: '[鉂梋 Este comando s贸 pode ser usado pelo grupo propriet谩rio! 鉂�',
					ownerB: '[鉂梋 Este comando s贸 pode ser usado pelo Toin! 鉂�',
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '鈺斺晲鈺愨湭銆� Mencionando Todos 銆欌湭鈺愨晲\n鈺犫灔'+teks+'鈺氣晲銆� mqz BOT 銆�', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*銆� BROADCAST 銆�*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*銆� BROADCAST 銆�*\n\n${body.slice(4)}`)
						}
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (Number(args[0]) === 1) {
						...
						reply('鉂� SUCESSO 鉂� prontinho porno liberado fml')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('鉂� SUCESSO 鉂� porno desativado fml adm chato')
					} else {
						reply('digite 1 para ativar, 0 para desativar o recurso')
					}
					break	
