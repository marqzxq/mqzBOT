...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Calmae garai to fazendo aq ⌛',
				success: '✔️ Sucesso ✔️',
				...
				error: {
					stick: '[❗] Falha, ocorreu um erro ao converter a imagem em um adesivo, tente novamente ❌',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nOlá,!\nVocê não está registrado no banco de dados,porfavor registre-se para utilizar o bot \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Toin|18`,
				}
			}
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 mqz BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ SUCESSO ❭ prontinho porno liberado fml')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('❬ SUCESSO ❭ porno desativado fml adm chato')
					} else {
						reply('digite 1 para ativar, 0 para desativar o recurso')
					}
					break	
